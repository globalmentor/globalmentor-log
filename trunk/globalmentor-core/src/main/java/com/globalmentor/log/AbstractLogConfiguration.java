 ...
 * <p>
 * This implementation uses a concurrent map which is thread-safe but still allows race conditions. It is assumed that it is benign if multiple loggers get
 * created temporarily for a class.
 * </p>
 * <p>
 * The logger created in {@link #createLogger(Class)} should take into consideration the key returned by {@link #getLoggerKey(Class)} if appropriate.
 * </p>
...
public abstract class AbstractLogConfiguration<K> extends AbstractConfiguration implements LogConfiguration {
	...
	/**
	 * Sets the common logger. The common logger will be used if using a common logger is supported and no specific logger registrations have been made.
	...
	/**
	 * Returns the current common logger. If no common logger has been designated, one is created and configured.
	 * <p>
	 * This implementation creates a common logger by delegating to {@link #createLogger(Class)} passing the class for {@link Object}.
	 * </p>
	 ...
	 * <p>
	 * The actual object with which the logger is registered depends on the key determined for the class.
	 * </p>
	...
	/**
	 * Registers a logger for a given class/interface and all its sublasses/implementations that don't have specific logger registrations. If a logger is already
	 * configured for the given class, it will be replaced.
	 * <p>
	 * The actual object with which the logger is registered depends on the key determined for the class.
	 * </p>
	...
	/**
	 * Unregisters a logger for a given class/interface. If no logger is registered for a given class, no action is taken.
	 * <p>
	 * The actual object with which the logger is registered depends on the key determined for the class.
	 * </p>
	 ...
	 * <p>
	 * The returned logger may be a common logger, or it may be a logger configured for the calling class.
	 * </p>
	 * <p>
	 * This implementation locates the logger quickly if no class-specific loggers have been configured and use of the common logger is supported. Otherwise, a
	 * logger is returned for the calling class by delegating to {@link #getLogger(Class)}.
	 * </p>
	 ...
	 * <p>
	 * This implementation locates the logger quickly if no class-specific loggers have been configured and use of the common logger is supported. Otherwise, this
	 * implementation determines a logger in the following manner: If there is no logger configured for the specific class, a logger is searched for using each
	 * ancestor class and interface of the given class. If no logger is found for any superclass or interface, a logger is created using
	 * {@link #createLogger(Class)}. If there was no logger configured for the specific class, the determined logger will be associated with the specific class
	 * for faster lookups in the future.
	 * </p>
